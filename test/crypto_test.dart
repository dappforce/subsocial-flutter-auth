...
void main() {
  test('test hash verifyHash', () async {
    ...
    final hashedBytes = await crypto.hash(
      plain: plainBytes,
      salt: saltBytes,
    ...
    final verifyResultTrue = await crypto.verifyHash(
      plain: plainBytes,
    ...
    final verifyResultNotTheSamePlain = await crypto.verifyHash(
    ...
    final verifyResultNotTheSameSalt = await crypto.verifyHash(
      plain: plainBytes,
      expectedHash: hashedBytes,
  ...
  test('test encrypt decrypt', () async {
    ...
    // drive the aes key
    final key = await crypto.hash(
      plain: passwordBytes,
      salt: passwordSaltBytes,
      outputLength: keyLength,
    );

    final encryptedBytes = await crypto.encrypt(key: key, plain: plainBytes);

    expect(encryptedBytes, isNot(equals(plainBytes)));

    final decryptedBytes =
        await crypto.decrypt(key: key, cipher: encryptedBytes);
